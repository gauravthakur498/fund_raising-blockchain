...
beforeEach(async ()=>{
    ...
    factory=await new web3.eth.Contract(JSON.parse(compiledCampaign.interface))
...
describe('Campaign',()=>{
    ...
    it("allows people to invest and marked them as approvers",async()=>{
    ...
    it("allows a manger to make a payment request",async()=>{
        await campaign.methods.createRequest("Buy screen",'100',accounts[1])
        .send({
            from:accounts[0],
            gas:'1000000'
        });
        const request=await campaign.methods.request(0).call();
        assert.equal('Buy screen', request.description);
    });
